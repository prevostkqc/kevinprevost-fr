...
export default {
  ...
  props: {
    ...
    methods: {
        ...
        animate() {
            const now = Date.now();
            if (now - this.lastFrameTime < 16) { // Limite à environ 60 FPS
        ...
        generateParticles() {
            ...
            // Si les particules dépassent maxParticles, supprimez les plus anciennes
            if (this.particles.length > this.maxParticles) {
...
class Particle {
    constructor(x, y, angle, speed, size) {
        this.x = x;
        this.y = y;
        this.size = Math.min(size, 20); // Limiter la taille maximale
        this.angle = angle;
        this.speed = Math.min(speed, 2); // Limiter la vitesse
        this.alpha = 1;
        this.color = {
    ...
    update() {
        ...
        this.alpha -= 0.04; // Décroissance plus rapide pour libérer la mémoire
    }
    draw(ctx) {
        ctx.save();
