<template>
    ...
    <section :class="['kp_z-index', 'kp_changed__id', `kp_${context}`, 'kp_item__window']" :id="`kp_${context}`">
        <div class="kp_element--title">
            ...
            <Navigation :viewName="title" :context="context" :customClass="customClass" />
        </div>
  ...
  props: {
    ...
    mounted() {
        this.$nextTick(() => {
            ...
            console.error('Element terminalText not found');
            }
        });
    },
...
<style scoped>
  ...
  @keyframes blinkText {
    ...
    50%{border-right: solid 1px black;}
    100%{border-right: solid 1px white;}
  }
/* Fin tgerminal */
